...
export class WirelessADBManager {
   ...
   * 选择连接方式
   */
  private async promptConnectionMethod(): Promise<
    "wireless-debug" | "tcpip" | null
  > {
    const items = [
      {
        label: "无线调试",
  ...
  private async loadWirelessDevices(): Promise<SavedWirelessDevice[]> {
    try {
      const saved = this.context.globalState.get<SavedWirelessDevice[]>(
        this.STORAGE_KEY,
        [],
      );
  ...
  private async attemptReconnect(
    savedDevice: SavedWirelessDevice,
  ): Promise<boolean> {
  ...
  async disconnectDevice(device: WirelessDevice): Promise<void> {
    ...
    try {
      ...
      this.wirelessDevices = this.wirelessDevices.filter(
        (d) => d.id !== device.id,
      );

      vscode.window.showInformationMessage(
        `已断开: ${device.model || endpoint}`,
      );
      this.onDidChangeDevicesEmitter.fire();
    } catch (error: any) {
